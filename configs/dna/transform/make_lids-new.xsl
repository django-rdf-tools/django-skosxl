...
<!--                    <xsl:apply-templates select="." mode="footer"/>-->
    ...
    <xsl:template match="item" mode="views">
        <xsl:param name="scope">set</xsl:param>
        <xsl:param name="id"/>
        <xsl:variable name="view" select="viewName"/>
        <xsl:variable name="vt" select="viewType/@href"/>
        
        <xsl:if test="(count(featurescope) = 0 ) or featurescope = $scope">

            <TR>
                <TD>
                    <xsl:choose>
                        <xsl:when test="count(viewType) = 1" >
    ...
    <xsl:template match="hasFormat">
        <xsl:param name="view"/>
        <xsl:param name="id"/>
        <!--      <xsl:variable name="format" select="ldatoken"/>-->
    ...
    <xsl:template name="format">
        ...
        <xsl:param name="id"/>
        <!--      <xsl:variable name="format" select="ldatoken"/>--> [<a
    ...
    <xsl:template name="se">
        <xsl:variable name="sp" select="//sparqlendpoint/@href"/>
